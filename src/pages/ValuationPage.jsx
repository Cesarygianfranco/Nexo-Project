...
const ValuationPage = () => {
		...
		Promise.all([
			...
			.then(([categoriesRes, productsRes]) => {
				const categoriesData = categoriesRes.data
					? Object.keys(categoriesRes.data).map((id) => ({
							id,
							...categoriesRes.data[id],
						}))
					: [];

				const productsData = productsRes.data
					? Object.keys(productsRes.data).map((id) => ({
							id,
							...productsRes.data[id],
						}))
					: [];
	...
	const grandTotal = products.reduce(
		(acc, product) => acc + product.amount * product.value,
		0,
	);
			...
			<Stack gap="xl">
				...
				{categories.map((category) => {
					const categoryProducts = products.filter(
						(products) => products.categoryId === category.id,
					);
								...
								<Table.Tbody>
									{categoryProducts.map((product) => (
										<Table.Tr key={product.id}>
											...
											<Table.Td ta="right">{product.amount}</Table.Td>
											<Table.Td ta="right">
				...
				>
					<Group justify="space-between">
						<Stack gap={0}>
							...
							<Text opacity={0.8}>
								Calculated based on current stock levels
							</Text>
						</Stack>
